...
/// Find the `packages/netcode` root by walking up from [start] until a directory
...
Directory _findPackageRoot(Directory start) {
  ...
  // Fallback: look for packages/netcode from cwd
  final cwd = Directory.current;
  final candidate = Directory('${cwd.path}/packages/netcode');
  if (candidate.existsSync()) return candidate;
  throw StateError('Cannot find packages/netcode root from ${start.path}');
}

void main() {
  test('packages/netcode/lib is platform-pure and determinism-safe', () {
    // Resolve lib/ robustly: walk up from cwd until we find a dir with
    // pubspec.yaml + lib/, which is the packages/netcode root.
    ...
    expect(offenders, isEmpty,
        reason: 'determinism/purity violations in packages/netcode/lib:\n${offenders.join('\n')}');
  });
}
