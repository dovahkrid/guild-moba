...
- **No client-side death-barrier is needed.** The pre-death held order is acked and pruned from `_pending` (`match_controller.dart:170`) within a few ticks of being sent — long before the 150-tick (5 s) respawn — and the authoritative standing state (`target = pos`, `attackTargetId = -1`) is carried in snapshots and inherited via `restoreFromSnapshot`. Across the long downed window `_pending` holds no live order, so `_intentsAt` returns nothing and the predicted hero stands. A TDD netcode test proves this (kill the local hero, run past respawn, assert it stands with reconcile correction ≈ 0). *(Fallback if a test ever shows divergence: add an explicit t-aware death-barrier in `_intentsAt`. Not anticipated.)*
